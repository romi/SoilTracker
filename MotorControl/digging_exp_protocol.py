...
while abs(xAxis.encoder.pos_estimate - startPositionX) * grX < 30: # the tool will go 60cm "forwards"
    continue
...
stopCondition = False
currentZAxis_noDig = DataContainer()
currentZAxis_dig = DataContainer()
currentWAxis_dig = DataContainer()
currentWAxis_noDig = DataContainer()
velWAxis_noDig = DataContainer()
velWAxis_dig = DataContainer()
posZ_noDig = DataContainer()
...
while abs((zAxis.encoder.pos_estimate - startPositionZ) * grZ) < 25 and stopCondition == False: # the tool will (try to) go 25cm deep
    currentZAxis_noDig.measure(zAxis.motor.current_control.Iq_measured)
    currentWAxis_noDig.measure(wAxis.motor.current_control.Iq_measured)
    current_overload = (abs(current_filter.avg(currentZAxis_noDig.filtered[-150:])) > 6.5)
    zRevolutions = zAxis.encoder.pos_estimate                                   # estimated position in revolutions
    zPos = (zRevolutions - depth) * grZ                                         # estimated position of the tool on the Z-axis1
    posZ_noDig.measure(zPos)
    velWAxis_noDig.measure(wAxis.encoder.vel_estimate)
    if current_overload or len(currentZAxis_noDig.t) > 20_000:
        if currentZAxis_noDig.t[-1] > 1:
            stopCondition = True
...
depth = zAxis.encoder.pos_estimate - (5/grZ)
input("Rebuild the terrain and press enter")
stopCondition = False
zAxis.controller.config.control_mode = CONTROL_MODE_VELOCITY_CONTROL
zAxis.controller.input_vel = -2 * grZ
...
SAVE_PATH = "ExpData/NoXMovement/"
prefixes = ("currentZAxis_noDig", "currentZAxis_dig", "currentWAxis_dig", "currentWAxis_noDig", "velWAxis_noDig", "velWAxis_dig", "posZ_noDig", "posZ_dig")
datasets = (currentZAxis_noDig, currentZAxis_dig, currentWAxis_dig, currentWAxis_noDig, velWAxis_noDig, velWAxis_dig, posZ_noDig, posZ_dig)

for prefix, data in zip(prefixes, datasets):
    data.save(SAVE_PATH+prefix)
