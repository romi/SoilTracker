				...
				if (getPacketReturnCode(tracker, &res) == EPHIDGET_OK) {
					...
					if (res != EPHIDGET_OK) {
						...
						if (trans->error == EPHIDGET_OK) {
							trans->error = res;
							logdebug("%"PRIphid" Got a packet error during a multi-packet transaction: "PRC_FMT, device, PRC_ARGS(res));
						} else {
							logwarn("%"PRIphid" Got another packet error (total: %d) during a multi-packet transaction: "PRC_FMT, device, trans->errorCnt, PRC_ARGS(res));
						}
					}
				}
